        ...
        public static string Collect()
        {
            ...
            if (OperatingSystem.IsWindows())
            {
                try
                {
                    ...
                    // Pre-fetch ALL CPU performance data in ONE WMI query
                    Dictionary<int, int> cpuUsageByPid = new Dictionary<int, int>();
                    try
                    {
                        using (ManagementObjectSearcher perfSearcher = new ManagementObjectSearcher("root\\CIMV2", 
                            "SELECT IDProcess, PercentProcessorTime FROM Win32_PerfFormattedData_PerfProc_Process"))
                        {
                            foreach (ManagementObject perfObj in perfSearcher.Get())
                            {
                                try
                                {
                                    int perfPid = Convert.ToInt32(perfObj["IDProcess"]);
                                    int cpuPercent = Convert.ToInt32(perfObj["PercentProcessorTime"]) / processorCount;
                                    cpuUsageByPid[perfPid] = cpuPercent;
                                }
                                catch { }
                            }
                        }
                    ...
                    {
                        Logging.Device_Information("Device_Information.Process_List.Collect", "Failed to prefetch CPU data.", ex.Message);
                    }

                    // Pre-fetch parent process names to avoid repeated Process.GetProcessById calls
                    Dictionary<int, string> processNamesByPid = new Dictionary<int, string>();
                    try
                    {
                        foreach (Process proc in Process.GetProcesses())
                        {
                            processNamesByPid[proc.Id] = proc.ProcessName;
                        }
                    }
                    catch { }
                    ...
                    using (ManagementObjectSearcher searcher = new ManagementObjectSearcher("root\\CIMV2", "select * from Win32_Process"))
                    {
                        foreach (ManagementObject reader in searcher.Get())
                        {
                            try
                            {
                                ...
                                // Use pre-fetched CPU data instead of per-process WMI query
                                int cpu_percentage = cpuUsageByPid.TryGetValue(pid, out int cpuVal) ? cpuVal : 0;
                                
                                int parent_id = Convert.ToInt32(reader["ParentProcessId"]);
                                // Use pre-fetched parent names instead of Process.GetProcessById
                                string parent_name = processNamesByPid.TryGetValue(parent_id, out string pName) ? pName : "";
                                ...
                                try { user = Helper.Process_User.Get(process_info) + " (" + process_info.SessionId + ")"; } catch { }
                                try { created = process_info.StartTime.ToString(); } catch { }
                                try { path = process_info.MainModule?.FileName ?? ""; } catch { }
                                ...
                                // RAM usage in MB - use WorkingSet64 directly instead of PerformanceCounter (much faster)
                                try
                                {
                                    ram_usage = Math.Round((double)process_info.WorkingSet64 / (1024 * 1024), 2);
                                }
                                catch { }
